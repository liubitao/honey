...
- (void)setup{
    ...
    NSString *str = [NSString stringWithFormat:@"获奖奖品：%@",_model.productName];
    _productNameLabel.attributedText = [Utils stringWith:str font1:SYSTEM_FONT(12) color1:UIColorHex(969696) font2:SYSTEM_FONT(12) color2:UIColorHex(5cb0e5) range:NSMakeRange(5, _model.productName.length)];
    ...
    _periodLabel.text = [NSString stringWithFormat:@"商品期数：%@",_model.productPeriod];
    ...
    _participateLabel.text = [NSString stringWithFormat:@"本期参与：%@",_model.participate];
    ...
    _luckyNumberLabel.text = [NSString stringWithFormat:@"幸运号码：%@",_model.luckyNumber];
    ...
    _publishTimeLabel.text = [NSString stringWithFormat:@"揭晓时间：%@",[NSString transToTime:_model.publishTime]];
...
- (void)setup{
    ...
    [_headerImage sd_setImageWithURL:[NSURL URLWithString:_model.headImageUrl]];
    ...
    _timeLabel.origin = CGPointMake(_headerImage.right+10, _usernameLabel.bottom);
    _timeLabel.size = CGSizeMake(self.width- 120, 13);
    _timeLabel.font = SYSTEM_FONT(12);
    _timeLabel.textColor = UIColorHex(969696);
    _timeLabel.text = [NSString transToTime:_model.publishTime];
    ...
    [_zanButton setImage:[UIImage imageNamed:@"zan"] title:[NSString stringWithFormat:@"(%@)",_model.number] forState:UIControlStateNormal];
    ...
    for (int i = 0; i<_model.imageList.count; i++) {
        [self addImageView:self.height url:_model.imageList[i]];
    }
}
...
- (void)addImageView:(CGFloat)top url:(NSString *)urlStr{
    ...
    self.height = imageView.bottom +5;
