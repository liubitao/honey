...
- (void)setModel:(KHWinCodeModel *)model{
    ...
    if (model.order_status.integerValue == 0 ||model.order_status.integerValue == 1) {//待确认
        ...
        if (model.order_status.integerValue == 0) {
          [_confirmButton setTitle:@"确认地址" forState:UIControlStateNormal];
            _stateType = KHWinCodeStateConfirm;
        }else{
           [_confirmButton setTitle:@"等待发货" forState:UIControlStateNormal];
             _stateType = KHWinCodeStateNODeliver;
        }
    }else{
